...
def write_docx(path: Path, pages: list[list[str]], source_page_numbers: list[int], facts: dict, label: str) -> None:
    doc = Document()
...
def main() -> int:
    ...
    if total_pages < 60:
        ...
        selected_indices = list(range(30)) + list(range(total_pages - 30, total_pages))
        selection = "first_30_and_last_30"
    filing_pages = [full_pages[index] for index in selected_indices]
    source_page_numbers = [index + 1 for index in selected_indices]
    output = args.output_dir.resolve()
    output.mkdir(parents=True, exist_ok=True)
    full_txt = output / "source-full.txt"
    filing_txt = output / "source-filing.txt"
    full_docx = output / "source-full.docx"
    filing_docx = output / "source-filing.docx"
    write_txt(full_txt, full_pages)
    write_txt(filing_txt, filing_pages)
    write_docx(full_docx, full_pages, list(range(1, total_pages + 1)), facts, "完整归档版")
    write_docx(filing_docx, filing_pages, source_page_numbers, facts, "申报版")
    ...
    provenance = {
        ...
        "filing_page_count": len(filing_pages),
        "selection": selection,
        "selected_source_pages": source_page_numbers,
        "files": file_records,
        "pages": line_pages,
        "line_mapping": mapping,
        "artifacts": {
            "full_txt": {"path": str(full_txt), "sha256": sha256_file(full_txt)},
            "filing_txt": {"path": str(filing_txt), "sha256": sha256_file(filing_txt)},
            "full_docx": {"path": str(full_docx), "sha256": sha256_file(full_docx)},
            "filing_docx": {"path": str(filing_docx), "sha256": sha256_file(filing_docx)}
        }
    }
    provenance_path = output / "source-provenance.json"
    save_json(provenance_path, provenance)
    print(f"SOURCE_OUTPUT={output}")
    print(f"FILES={len(file_records)} LINES={len(lines)} FULL_PAGES={total_pages} FILING_PAGES={len(filing_pages)} SELECTION={selection}")
